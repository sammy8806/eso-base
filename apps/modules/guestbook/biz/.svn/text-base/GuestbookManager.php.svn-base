...
class GuestbookManager extends APFObject {
   ...
   public function saveEntry(Entry $entry) {
      ...
      $link = LinkGenerator::generateUrl(Url::fromCurrent()->mergeQuery(array($urlParams['PageName'] => '', $urlParams['CountName'] => '', 'gbview' => 'display')));
   ...
   public function saveComment($entryID, Comment $comment) {
      ...
      // forward to the target page
      $urlParams = $this->getURLParameters();
      $link = LinkGenerator::generateUrl(Url::fromCurrent()->mergeQuery(array($urlParams['PageName'] => '', $urlParams['CountName'] => '', 'gbview' => 'display', 'commentid' => '', 'entryid' => '')));
      HeaderManager::forward($link);
   }
